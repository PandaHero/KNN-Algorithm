...
def get_product_list_info():
    ...
    for item in product_items:
        ...
        if productPrice:
            # 商品价格
            productPrice = float(productPrice.get_text().replace("¥", "").strip())
            # 商品名称
            ...
            if "万" in month_buy:
                monthBuy = int(10000 * float(month_buy.replace("万", "")))
            else:
                monthBuy = int(month_buy)
            # 评价数
            productComments = productStatus.split("笔")[1].replace("评价", "").strip()
            if "万" in productComments:
                productComments = int(10000 * float(productComments.replace("万", "")))
            else:
                productComments = int(productComments)
            product_info = {
...
#
# # 解析商品详情页信息(302暂未解决)
# def get_detail_product_info(productUrl, count=1):
...
#             }
#             # 若存在代理，则使用代理
#             response = requests.get(productUrl, allow_redirects=False, headers=headers, proxies=proxies)
#         else:
...
#                 count += 1
#                 return get_detail_product_info(productUrl, count)
#             else:
#                 print("get proxy failed")
#                 return None
#     except ConnectionError:
#         print("error occured ", ConnectionError.args)
#         proxy = get_proxy()
#         count += 1
#         return get_detail_product_info(productUrl, count)
...
def main():
    ...
    # 创建数据库对象
    my_sql_helper = src.mysqlHelper.MysqlHelper("localhost", "root", "root", "mypydb")
    # 连接
    my_sql_helper.connect()
    for page in range(1, total_page_num + 1):
        next_page(page)
        product_info_list = get_product_list_info()
        for product_info in product_info_list:
            # 写入mysql的时候数据类型是%s，不然报错(情况未知)
            sql = "INSERT INTO tmall_product_price values (%s,%s,%s,%s,%s,%s,%s,%s,%s)   "
            my_sql_helper.insert(sql, (
                product_info["productTitle"], product_info["productUrl"], product_info["productPrice"],
                product_info["monthBuy"], product_info["productComments"], product_info["productShop"],
                product_info["spu"], product_info["sku"], product_info["shopID"]))
            my_sql_helper.connect()
        # threads = []
        # for product_info in product_info_list:
